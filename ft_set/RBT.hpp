...
Node<value_type,Alloc>* RBT<value_type,Compare, Alloc>::Insert_helper(_NodeType *node, _NodeType **in_node) // smashes stack 
{
		...
		if (!this->_comp_obj(*(node->_p), value) && !this->_comp_obj(value, *(node->_p)))
		{
				delete *in_node; // No insertion performed
				*in_node = node;
				return (node);
		}
		side = this->_comp_obj(*(node->_p), value);
...
template<typename value_type, typename Compare, typename Alloc> 
typename RBT<value_type,Compare,Alloc>::_NodeType *RBT<value_type,Compare,Alloc>::find_node(const value_type &key) const
{
		bool side;
		...
		while (node)
		{
				if (!this->_comp_obj(*(node->_p), key) && !this->_comp_obj(key, *(node->_p)))
						return (node);
				side = this->_comp_obj(*(node->_p), key);
...
template<typename value_type, typename Compare, typename Alloc> 
typename RBT<value_type,Compare,Alloc>::_NodeType *RBT<value_type,Compare,Alloc>::get_bound(const value_type &key, bool bound) const
{
		bool side;
		bool upper(1);
		bool cond;
		...
		while (node)
		{
				cond = this->_comp_obj(key, *(node->_p));
				if (bound == upper && cond)
						return (node);
...
Node<value_type, Alloc>* RBT<value_type,Compare,Alloc>::DeleteHelper(_NodeType *node, value_type *data, bool &isBalanced)
{
		...
		dir = this->_comp_obj(*(node->_p), *data); 
